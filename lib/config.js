var _ = require('lodash'),
    ...
    configFile = require('../lib/constants').configFilename,
    ...
    fs.readFile(path.join(templateDir, CONFIG_FILE), function(err, file){
        if(err) {
            ...
            if(!vars){
                ...
                var varMapping = JSON.parse(file.toString()).boiler.var_mapping;
...
Config.prototype.getFiles = function(){
    ...
    readdirp({ 'root': this.templateDir,
        ...
        // FINITO!
        that.completeFn(null, that.dirs, that.files, that.vars, that.data.boiler.output_file_dir);
    });
};

Config.prototype.getFileMapping = function(){
    // get file mapping
    if(this.data.boiler.file_mapping !== undefined){
        if(_.isArray(this.data.boiler.file_mapping)){
            this.file_mapping = this.data.boiler.file_mapping;
        }
    }
};

Config.prototype.getFilters = function(){
    if(this.data.boiler.file_filter) this.fileFilter = this.data.boiler.file_filter;
    if(this.data.boiler.dir_filter) this.dirFilter = this.data.boiler.dir_filter;
