...
var _getPackageName = function(onComplete){
    ...
    prompt.get(packageName, function(err, result){
        ...
        config.packageName = result.packageName;
        onComplete();
    });
};
        ...
        if(!hasMapping){
            ...
            prompt.get(schema, function(err, result){
                ...
                if(result.varName && (result.varDesc || result.varType || result.defaultVar)){
                    config.var_mapping[result.varName] = {};
                    var obj = config.var_mapping[result.varName];
...
var _getOutputDir = function(callback){
    ...
    prompt.get(schema, function(err, result){
        config.output_file_dir = result.outputDir;
        callback();
    });
};
    ...
    _getYesNo(function(isAccepted){
        if(isAccepted){
            ...
            fs.writeFile(configFileName, JSON.stringify(config, null, 4), function(err){
                ...
                console.log('Install this boiler by using the command ' + '`boiler install`'.magenta);
            });
        }else{
            _execute();
        }
